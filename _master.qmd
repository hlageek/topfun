...
As topic model requires some data pre-processing, we make it is own subproject.

- __Run__: Needs to be run only once, unless the input data changes.
- __Requirement__: 
...
#| eval: false
quarto::quarto_render("_topmodel.qmd", quiet = TRUE)
targets::tar_make(script = "_topmodel.R", store = "_topmodel")
```
