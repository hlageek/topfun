...
The orchestration file can be used to sequentally compile individual subprojects from a single space.
...
The two sections are only connected via a _pinboard_ of data that are exported in @sec-data one and reimported in @sec-analysis. 
As a consequence, only the project administrator needs to run the first section and collaborators can focus on the subprojects in section two.
...
```
### Produce topic model {#sec-data_topmodel}
